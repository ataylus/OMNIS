...
# The synthetic bench references the requirement ids parsed from the synthetic
# policy file (the 3 provided policies plus 3 added ones: SOX, HIPAA, PCI-DSS;
# 15 requirements total), so mapping and scoring tell one coherent story on the
# synthetic side (exact-id mapping works; INCOMPLETE_MAPPING rows use ORPHAN-*
# ids that match no requirement, as before). The provided sample (3 policies, 9
# requirements) is unchanged and remains the primary evaluation set.
DEFAULT_POLICY_PATH = "data/synthetic/policy_documents.txt"
...
The synthetic bench is scored against `data/synthetic/policy_documents.txt`,
which holds 6 policies and {len(bench.valid_requirement_ids)} requirements: the 3
provided policies (POL-ENC-001, POL-AC-001, POL-AUD-001) verbatim, plus 3 added
policies in the same structured format (POL-SOX-001, POL-HIPAA-001,
POL-PCI-001). This restores the advertised 6-framework enterprise scope so the
report and dashboard show 12-15 requirements. The provided sample (3 policies, 9
requirements) is unchanged and remains the primary evaluation set.
...
low-confidence threshold = {LOW_CONFIDENCE_THRESHOLD}. Mappable rows reference the
{len(bench.valid_requirement_ids)} requirement ids parsed from the synthetic policy
file `data/synthetic/policy_documents.txt` (the 3 provided policies plus 3 added
ones: SOX, HIPAA, PCI-DSS); INCOMPLETE_MAPPING rows reference ORPHAN-* ids.
